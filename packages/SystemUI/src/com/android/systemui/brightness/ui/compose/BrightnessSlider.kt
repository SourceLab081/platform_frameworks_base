...
fun BrightnessSlider(
    ...
    val customRadiusDp = rememberSliderCustomCornerRadius().dp
    val trackCornerDp: Dp = when (shapeMode) {
        1 -> 24.dp  /* Circle */
        2 -> 12.dp  /* Rounded Square */
        3, 4 -> customRadiusDp /* Custom */
...
@Composable
fun rememberSliderCustomCornerRadius(): Int {
    val context = LocalContext.current
    val contentResolver = context.contentResolver

    fun readCustomRadius(): Int {
        return try {
            Settings.System.getIntForUser(
                contentResolver, Settings.System.QS_BRIGHTNESS_SLIDER_SHAPE_CORNER_RADIUS, 16,
                UserHandle.USER_CURRENT
            )
        } catch (_: Throwable) {
            16
    ...
    var customRadius by remember { mutableIntStateOf(readCustomRadius()) }

    DisposableEffect(contentResolver) {
        val observer = object : ContentObserver(null) {
            override fun onChange(selfChange: Boolean) {
                context.mainExecutor.execute {
                    customRadius = readCustomRadius()
                }
            }
        }

        contentResolver.registerContentObserver(
            Settings.System.getUriFor(Settings.System.QS_BRIGHTNESS_SLIDER_SHAPE_CORNER_RADIUS),
            false, observer, UserHandle.USER_ALL
        )
...
private fun drawAutoBrightnessButton(
    ...
    val autoIconShape = when (shapeMode) {
        ...
        3, 4 -> RoundedCornerShape(customRadiusDp)
        else -> CircleShape
    }
