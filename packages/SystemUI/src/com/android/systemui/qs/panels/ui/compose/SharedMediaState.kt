...
internal fun rememberMediaState(): SharedMediaState {
    ...
    DisposableEffect(Unit) {
        ...
        val listenerComponent = ComponentName(context, NotificationListenerWithPlugins::class.java)

        fun refreshActiveSessions() {
            val sessions: List<MediaController> = try {
                sessionManager?.getActiveSessions(listenerComponent) ?: emptyList()
            ...
            if (currentController !== active) {
                ...
                if (active != null) {
                    ...
                    active.registerCallback(cb, mainHandler)
