...
constructor(
    ...
    private val _isBrightnessSliderInternal by
    ...
    val isBrightnessSliderVisible: Boolean
        get() = _isBrightnessSliderInternal && !isWidgetPanelEnabled
    ...
    val showMedia: Boolean
        get() = mediaCarouselInteractor.hasAnyMedia.value && !isWidgetPanelEnabled
    ...
    var isWidgetPanelEnabled by mutableStateOf(false)
        private set

    private val widgetPanelObserver: ContentObserver by lazy {
        val cr = context.contentResolver
        val handler = Handler(Looper.getMainLooper())
        object : ContentObserver(handler) {
            override fun onChange(selfChange: Boolean) {
                isWidgetPanelEnabled = try {
    ...
    override suspend fun onActivated() {
        val cr = context.contentResolver
        widgetPanelObserver.onChange(true)
        cr.registerContentObserver(
            Settings.System.getUriFor(Settings.System.QS_WIDGET_PANEL),
            false, widgetPanelObserver, UserHandle.USER_ALL,
        )
    ...
    override suspend fun onDeactivated() {
        context.contentResolver.unregisterContentObserver(widgetPanelObserver)
    }
