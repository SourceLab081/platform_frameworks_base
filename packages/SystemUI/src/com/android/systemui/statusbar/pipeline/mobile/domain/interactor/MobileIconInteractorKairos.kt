...
class MobileIconInteractorKairosImpl(
    ...
    private val showMobileDataDisableIcon: State<Boolean> = buildState {
        conflatedCallbackFlow {
            val observer = object : ContentObserver(null) {
                override fun onChange(selfChange: Boolean) {
                    val value = Settings.Secure.getInt(
                        context.contentResolver,
                        Settings.Secure.STATUS_BAR_MOBILE_DATA_DISABLE_ICON,
                        1
                    )
                    trySend(value == 1)
                }
            ...
            context.contentResolver.registerContentObserver(
                Settings.Secure.getUriFor(Settings.Secure.STATUS_BAR_MOBILE_DATA_DISABLE_ICON),
                false,
                observer
            )
            val initialValue = Settings.Secure.getInt(
                context.contentResolver,
                Settings.Secure.STATUS_BAR_MOBILE_DATA_DISABLE_ICON,
                1
            )
            trySend(initialValue == 1)
            awaitClose {
                context.contentResolver.unregisterContentObserver(observer)
            }
        }.toState(
            true,
            nameTag("MobileIconInteractorKairosImpl.showMobileDataDisableIcon")
        )
    }
    ...
    private val showExclamationMarkForCellular: State<Boolean> =
        combine(
            ...
            isInService,
            showMobileDataDisableIcon
        ) { isDefaultDataEnabled, isDefaultConnectionFailed, isInService, showDisableIcon ->
            (!isDefaultDataEnabled && showDisableIcon) || isDefaultConnectionFailed || !isInService
        }
