...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    private DoubleTapToSleepListener mDoubleTapToSleepListener;
    ...
    void init(Injector injector) {
        ...
        mDoubleTapToSleepListener = new DoubleTapToSleepListener(mContext, mWindowManagerInternal,
                mActivityTaskManagerInternal, mHandler);
    ...
    private void enableDoubleTapToSleepGesture(boolean enable) {
        if (enable) {
            if (mDoubleTapToSleepEnabled) return;
            mDoubleTapToSleepEnabled = true;
            mWindowManagerFuncs.registerPointerEventListener(mDoubleTapToSleepListener, DEFAULT_DISPLAY);
        } else {
            if (!mDoubleTapToSleepEnabled) return;
            mDoubleTapToSleepEnabled = false;
            mWindowManagerFuncs.unregisterPointerEventListener(mDoubleTapToSleepListener, DEFAULT_DISPLAY);
        }
    }
    ...
    void updateSettings() {
        ...
        synchronized (mLock) {
            ...
            // Double Tap to Sleep
            boolean doubleTapSleep = Settings.System.getIntForUser(resolver,
                    Settings.System.DOUBLE_TAP_SLEEP_GESTURE, 0, UserHandle.USER_CURRENT) == 1;
            enableDoubleTapToSleepGesture(doubleTapSleep);
